...
const service = {
  ...
  works: () => FIND_MANY_RESULT,
  work: ({ where }: { where: { id: string } }) => {
    switch (where.id) {
      case existingId:
...
describe("Work", () => {
  ...
  test("GET /works/:id non existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/works"}/${nonExistingId}`)
  ...
  test("GET /works/:id existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/works"}/${existingId}`)
      .expect(HttpStatus.OK)
      .expect({
        ...FIND_ONE_RESULT,
  ...
  test("POST /works existing resource", async () => {
